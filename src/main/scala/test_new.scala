...
def con(id: Int, name: String): Boolean = {
  existingProducts.contains(id)
}

artistByID.filter {case (id, name) =>
  existingProducts.contains(id.toInt)
}.values.collect().foreach(println)


artistByID.filter (con(_,_)).values.collect().foreach(println)
...
artistByID.filter (x=> existingProducts.contains(x._1)
).values.collect().foreach(println)
artistByID.filter (existingProducts.contains(_._1)
).values.collect().foreach(println)
